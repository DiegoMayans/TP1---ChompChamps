  ...
  for (int i = 0; i < alto; i++) {
    for (int j = 0; j < ancho; j++) {
      if (board_state->board[i * (board_state->width) + j] < 0) {
        ...
        if ((i == board_state->players_list[player].x) &&
            (j == board_state->players_list[player].y)) {
          printf("\033[38;5;%dm| %d |\033[0m", head_colors[player],
                 board_state->board[i * (board_state->width) + j]);
        } else {
          printf("\033[38;5;%dm| %d |\033[0m", colors[player],
                 board_state->board[i * (board_state->width) + j]);
        }
      } else {
